import{F as R,c as $,h as w}from"./highlight-BNcuc9BJ.js";const x=["json","jsonl","csv","tsv","yaml","xml"],y={json:`[
...
</users>`};function C(n){if(n===0)return"0 bytes";const t=Math.abs(n),r=t/(1024*1024),c=t/1024;if(t>=1024*1024){const o=Math.round(r*10)/10;return o===Math.floor(o)?`${Math.floor(o)}MB`:`${o}MB`}if(t>=1024){const o=Math.round(c*10)/10;return o===Math.floor(o)?`${Math.floor(o)}KB`:`${o}KB`}return`${Math.floor(t)} bytes`}function S(n,t,r){const c=document.getElementById("hero-size-info"),o=document.getElementById("hero-size-change");if(!t||!n){c.style.display="none";return}let d=t;if(r){const s=r.match(/:\s*/);if(!s)d=t;else{const e=s.index+s[0].length,a=r.indexOf(`
`,e)+1;let g=r.length;const u=r.split(`
`);for(let l=u.length-1;l>=0;l--)if(u[l].startsWith("#")&&!/^#\s+\d+\s+records?$/.test(u[l].trim())){g=r.lastIndexOf(u[l]);break}d=g-a}}const i=d-n,m=Math.round(i/n*100),h=i<0?"var(--success)":"var(--error)",f=i<0?"Data size saved":"Data size increased";o.innerHTML=`
    <span style="color:${h};">${f} <strong>${Math.abs(m)}%</strong> (<span style="color:${h};">${C(Math.abs(i))}</span>)</span>
  `,c.style.display="block"}function T(){const n=document.getElementById("hero-tabs"),t=document.getElementById("hero-input"),r=document.getElementById("hero-output"),c=document.getElementById("hero-convert"),o=document.getElementById("hero-swap"),d=document.getElementById("hero-delimiter"),i=document.getElementById("hero-sha256");let m="json";function h(e){m=e,t.value=y[e]??"",r.innerHTML="",r.classList.remove("error"),n.querySelectorAll(".format-tab").forEach(a=>{a.classList.toggle("active",a.dataset.fmt===e)})}function f(e,a=!1){r.classList.toggle("error",a),a?r.textContent=e:r.innerHTML=e?w(e):""}async function s(){try{const e={ingrDelimiter:d.checked};let a=$(t.value,m,"ingr",e);if(i.checked){const u=new TextEncoder().encode(a),l=await crypto.subtle.digest("SHA-256",u),M=Array.from(new Uint8Array(l)).map(B=>B.toString(16).padStart(2,"0")).join("");a=a+`
# sha256:`+M}f(a),S(new TextEncoder().encode(t.value).length,new TextEncoder().encode(a).length,a)}catch(e){f(e instanceof Error?e.message:String(e),!0)}}n.innerHTML=x.map(e=>`<button class="format-tab${e===m?" active":""}" data-fmt="${e}">${R[e]}</button>`).join(""),n.addEventListener("click",e=>{const a=e.target.closest(".format-tab");a?.dataset.fmt&&(h(a.dataset.fmt),s())}),c.addEventListener("click",s),d.addEventListener("change",s),i.addEventListener("change",s),o.addEventListener("click",()=>{const e=r.textContent??"";!e||r.classList.contains("error")||(t.value=e,r.innerHTML="",r.classList.remove("error"),h("ingr"))}),t.value=y.json,s()}document.addEventListener("DOMContentLoaded",T);const v="https://raw.githubusercontent.com/ingitdb/ingitdb-cli/refs/heads/main/docs/file-formats/ingr.md",U={parser:`Implement an INGR file format parser in [YOUR LANGUAGE].
