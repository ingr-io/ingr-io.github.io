...
`,s)}return i-c}return t.length}function k(t,n,r){const c=document.getElementById("hero-size-info"),o=document.getElementById("hero-size-change");if(!n||!t){c.style.display="none";return}let l=n;if(r){const m=r.match(/:\s*/);if(m){const g=m.index+m[0].length,f=r.split(`
`);let h=-1;for(let e=f.length-1;e>0;e--)if(f[e].startsWith("#")){h=e;break}let d=r.length;if(h>0){const e=r.indexOf(f[h]);d=r.lastIndexOf(`
`,e)}l=d-g}}const i=l-t,s=Math.round(i/t*100),p=i<0?"var(--success)":"var(--error)",u=i<0?"Data size saved":"Data size increased";o.innerHTML=`
    <span style="color:${p};">${u} <strong>${Math.abs(s)}%</strong> (<span style="color:${p};">${y(Math.abs(i))}</span>)</span>
  `,c.style.display="block"}function P(){const t=document.getElementById("hero-tabs"),n=document.getElementById("hero-input"),r=document.getElementById("hero-output"),c=document.getElementById("hero-convert"),o=document.getElementById("hero-swap"),l=document.getElementById("hero-delimiter"),i=document.getElementById("hero-sha256"),s=document.getElementById("hero-input-size"),p=document.getElementById("hero-output-size");let u="json";function m(e){u=e,n.value=L[e]??"",r.innerHTML="",r.classList.remove("error"),t.querySelectorAll(".format-tab").forEach(a=>{a.classList.toggle("active",a.dataset.fmt===e)}),g()}function g(){const e=I(n.value,u);s.textContent=e>0?`(${y(e)})`:""}function f(e){const a=I(e,"ingr");p.textContent=a>0?`(${y(a)})`:""}function h(e,a=!1){r.classList.toggle("error",a),a?r.textContent=e:r.innerHTML=e?T(e):""}async function d(){try{const e={ingrDelimiter:l.checked};let a=O(n.value,u,"ingr",e);if(i.checked){const C=new TextEncoder().encode(a),$=await crypto.subtle.digest("SHA-256",C),R=Array.from(new Uint8Array($)).map(w=>w.toString(16).padStart(2,"0")).join("");a=a+`
# sha256:`+R}h(a),k(new TextEncoder().encode(n.value).length,new TextEncoder().encode(a).length,a),f(a)}catch(e){h(e instanceof Error?e.message:String(e),!0)}}t.innerHTML=U.map(e=>`<button class="format-tab${e===u?" active":""}" data-fmt="${e}">${x[e]}</button>`).join(""),t.addEventListener("click",e=>{const a=e.target.closest(".format-tab");a?.dataset.fmt&&(m(a.dataset.fmt),d())}),c.addEventListener("click",d),l.addEventListener("change",d),i.addEventListener("change",d),n.addEventListener("input",g),o.addEventListener("click",()=>{const e=r.textContent??"";!e||r.classList.contains("error")||(n.value=e,r.innerHTML="",r.classList.remove("error"),m("ingr"))}),n.value=L.json,d()}document.addEventListener("DOMContentLoaded",P);const v="https://raw.githubusercontent.com/ingitdb/ingitdb-cli/refs/heads/main/docs/file-formats/ingr.md",z={parser:`Implement an INGR file format parser in [YOUR LANGUAGE].
