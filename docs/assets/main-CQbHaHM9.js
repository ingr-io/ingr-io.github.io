...
`);let e=s.length;for(let m=s.length-1;m>=1;m--)if(/^#\s+\d+\s+records?$/.test(s[m].trim())){e=m;break}const r=s.slice(1,e);r.length>0?c=new TextEncoder().encode(r.join(`
`)).length+1:c=0}const i=c-n,d=Math.round(i/n*100),u=i<0?"var(--success)":"var(--error)",h=i<0?"Data size saved":"Data size increased";o.innerHTML=`
