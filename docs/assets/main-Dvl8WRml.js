import{c as x,F as P,h as q}from"./highlight-CRuyUQB2.js";const O=["json","jsonl","csv","tsv","yaml","xml"],B={json:`[
  {
    "$id": "u1",
    "name": "Alice Chen",
    "role": "engineer",
...
</users>`};function N(t){if(t===0)return"0 bytes";const a=Math.abs(t),i=a/(1024*1024),u=a/1024;if(a>=1024*1024){const r=Math.round(i*10)/10;return r===Math.floor(r)?`${Math.floor(r)}MB`:`${r}MB`}if(a>=1024){const r=Math.round(u*10)/10;return r===Math.floor(r)?`${Math.floor(r)}KB`:`${r}KB`}return`${Math.floor(a)} bytes`}function k(t,a){if(!t||!a)return 0;if(a==="ingr"){const i=t.match(/:\s*/);if(!i)return 0;const u=i.index+i[0].length,r=t.split(`
...
`,o)}return d-u}return t.length}function _(t,a,i){const u=document.getElementById("hero-size-info"),r=document.getElementById("hero-size-change");if(!a||!t){u.style.display="none";return}let l=a;if(i){const p=i.match(/:\s*/);if(p){const y=p.index+p[0].length,h=i.split(`
...
`,s)}l=b-y}}const d=l-t,o=Math.round(d/t*100),c=d<0?"var(--success)":"var(--error)",L=d<0?"Data size saved":"Data size increased";r.innerHTML=`<span style="color:${c};">${L} <strong>${Math.abs(o)}%</strong> (<span style="color:${c};">${N(Math.abs(d))}</span>)</span>`,u.style.display="block"}function K(){const t=document.getElementById("hero-left-panel"),a=document.getElementById("hero-right-panel"),i=document.getElementById("hero-left-title"),u=document.getElementById("hero-right-title"),r=document.getElementById("hero-tabs"),l=document.getElementById("hero-right-tabs"),d=document.getElementById("hero-ingr-controls"),o=document.getElementById("hero-input"),c=document.getElementById("hero-output"),L=document.getElementById("hero-convert"),p=document.getElementById("hero-swap"),y=document.getElementById("hero-delimiter"),h=document.getElementById("hero-sha256"),g=document.getElementById("hero-input-size"),b=document.getElementById("hero-output-size");let s="json",m=!1,w="";function A(){r.innerHTML=O.map(e=>`<button class="format-tab${e===s?" active":""}" data-fmt="${e}">${P[e]}</button>`).join("")}function H(){l.innerHTML=O.map(e=>`<button class="format-tab${e===s?" active":""}" data-fmt="${e}">${P[e]}</button>`).join("")}function C(e){e.querySelectorAll(".format-tab").forEach(n=>{n.classList.toggle("active",n.dataset.fmt===s)})}function v(){const e=m?"ingr":s,n=k(o.value,e);g.textContent=n>0?`(${N(n)})`:""}function R(e){const E=k(e,m?s:"ingr");b.textContent=E>0?`(${N(E)})`:""}function $(e,n=!1){c.classList.toggle("error",n),n?(c.textContent=e,b.textContent=""):m?(c.textContent=e,R(e)):(c.innerHTML=e?q(e):"",R(e))}async function f(){try{if(m){const e=x(o.value,"ingr",s,{});$(e),document.getElementById("hero-size-info").style.display="none"}else{const e={ingrDelimiter:y.checked};let n=x(o.value,s,"ingr",e);if(h.checked){const E=new TextEncoder().encode(n),D=await crypto.subtle.digest("SHA-256",E),J=Array.from(new Uint8Array(D)).map(Y=>Y.toString(16).padStart(2,"0")).join("");n=n+`
# sha256:`+J}w=n,$(n),_(new TextEncoder().encode(o.value).length,new TextEncoder().encode(n).length,n)}}catch(e){$(e instanceof Error?e.message:String(e),!0)}}const S='<b style="color:var(--text);">.INGR</b>';function W(e){m?(i.innerHTML=`${S} input`,u.innerHTML='<b style="color:var(--text);">Output</b>',r.style.display="none",l.style.display="",d.style.display="none",o.placeholder="Paste INGR data here…",c.dataset.placeholder="Converted output will appear here…",e&&(o.value=e),H()):(i.innerHTML='<b style="color:var(--text);">Input</b>',u.innerHTML=`${S} output`,r.style.display="",l.style.display="none",d.style.display="",o.placeholder="Paste your data here…",c.dataset.placeholder="Converted INGR will appear here…",o.value=B[s]??"",A()),c.innerHTML="",c.textContent="",v(),f()}p.addEventListener("click",async()=>{const e=m?void 0:w;t.classList.add("is-swapping"),a.classList.add("is-swapping"),await new Promise(n=>setTimeout(n,180)),m=!m,W(e),t.classList.remove("is-swapping"),a.classList.remove("is-swapping")}),r.addEventListener("click",e=>{const n=e.target.closest(".format-tab");n?.dataset.fmt&&(s=n.dataset.fmt,C(r),o.value=B[s]??"",v(),f())}),l.addEventListener("click",e=>{const n=e.target.closest(".format-tab");n?.dataset.fmt&&(s=n.dataset.fmt,C(l),f())}),L.addEventListener("click",f),y.addEventListener("change",f),h.addEventListener("change",f),o.addEventListener("input",()=>{v(),m&&f()}),A(),o.value=B.json,v(),f()}document.addEventListener("DOMContentLoaded",K);const M="https://raw.githubusercontent.com/ingitdb/ingitdb-cli/refs/heads/main/docs/file-formats/ingr.md",V={parser:`Implement an INGR file format parser in [YOUR LANGUAGE].
...
Include usage examples for both parsing and building.`},U=document.getElementById("prompt-tabs"),G=document.getElementById("prompt-output"),I=document.getElementById("copy-prompt"),z=document.getElementById("lang-input");let j="parser";function T(){const t=z.value.trim()||"[YOUR LANGUAGE]";return V[j].replaceAll("[YOUR LANGUAGE]",t)}function F(t){j=t,G.textContent=T(),U.querySelectorAll(".format-tab").forEach(a=>a.classList.toggle("active",a.dataset.prompt===t))}U.addEventListener("click",t=>{const a=t.target.closest(".format-tab");a?.dataset.prompt&&F(a.dataset.prompt)});z.addEventListener("input",()=>{G.textContent=T()});I.addEventListener("click",async()=>{await navigator.clipboard.writeText(T());const t=I.innerHTML;I.innerHTML="<span>✓</span> Copied",setTimeout(()=>{I.innerHTML=t},1500)});F("parser");
